...
public class RunnerNodeModel extends ExtToolOutputNodeModel implements PortObjectHolder {
  ...
  private void createTopLevelJsonFile(CombinedFskPortObject fskObj,
      List<JoinRelationAdvanced> joinRelationList, ExecutionContext exec) throws Exception {
  ...
  private List<JoinRelationAdvanced> getMapOfTopLevelParameters(FskPortObject topLevel,
      FskPortObject fskObj, List<JoinRelationAdvanced> joinRelationList, String suffix) {
    if (joinRelationList == null) {
      joinRelationList = new ArrayList<JoinRelationAdvanced>();
    }
    if (fskObj instanceof CombinedFskPortObject) {
      joinRelationList = getMapOfTopLevelParameters(topLevel,
          ((CombinedFskPortObject) fskObj).getFirstFskPortObject(), joinRelationList,
          suffix + JoinerNodeModel.SUFFIX_FIRST);
      joinRelationList = getMapOfTopLevelParameters(topLevel,
          ((CombinedFskPortObject) fskObj).getSecondFskPortObject(), joinRelationList,
      ...
      for (Parameter topParam : topLevelParameter) {
        for (Parameter botParam : botLevelParameter) {
          if (topParam.getId().startsWith(botParam.getId() + suffix)) {
            JoinRelation joinRelation = new JoinRelation(botParam.getId(), topParam.getId(),
                botParam.getId() + suffix, SwaggerUtil.getLanguageWrittenIn(fskObj.modelMetadata));
  ...
  /**
   * Utility function to generate a Map that has the information of which parameter is joined with
   * which sourceParameter and where that source Parameter is (which model it belongs to). If he
   * join Relation between model1 and model2 is in2 <- out1 then the Map would append : < in2 ,
   * <model1, out >>
   * 
   * @param portObject FSKX model
   * @param joinRelations join relations of top-level combined model ( inlude all relations in a
   *        combined model)
   ...
   */
  private List<JoinRelationAdvanced> getMapOfSourceParameters(FskPortObject portObject,
      List<JoinRelation> joinRelations, List<JoinRelationAdvanced> joinRelationList,
      String suffix) {
    if (joinRelationList == null) {
      joinRelationList = new ArrayList<JoinRelationAdvanced>();
    }
  ...
  // simulation)
  public FskPortObject runFskPortObject(FskPortObject fskObj, FskSimulation combinedSim,
      ExecutionContext exec, List<JoinRelationAdvanced> joinRelationList, String suffix)
      throws Exception {
    ...
    if (fskObj instanceof CombinedFskPortObject) {
      ...
      if (!(firstFskObj instanceof CombinedFskPortObject)) {
        fskSimulationFirst = JoinerNodeUtil.makeIndividualSimulation(combinedSim, null, firstFskObj,
            suffix + JoinerNodeModel.SUFFIX_FIRST);
      } else {
        firstFskObj = stepIntoSubModel(firstFskObj, fskSimulationFirst, combinedSim, exec,
      ...
      FskSimulation fskSimulationSecond = JoinerNodeUtil.makeIndividualSimulation(combinedSim, null,
          secondFskObj, suffix + JoinerNodeModel.SUFFIX_SECOND);
      ...
      if (secondFskObj instanceof CombinedFskPortObject) {
        //
        secondFskObj = stepIntoSubModel(secondFskObj, fskSimulationSecond, combinedSim, exec,
            joinRelationList, suffix + JoinerNodeModel.SUFFIX_SECOND);
      } else {
        secondFskObj = runSnippet(secondFskObj, fskSimulationSecond, context, joinRelationList,
            suffix + JoinerNodeModel.SUFFIX_SECOND);
  ...
  private FskPortObject stepIntoSubModel(FskPortObject fskObj, FskSimulation fskSimulation,
      FskSimulation combinedSim, final ExecutionContext exec,
  ...
  private FskPortObject runSnippet(final FskPortObject fskObj, final FskSimulation simulation,
      final ExecutionContext exec, List<JoinRelationAdvanced> joinRelationList, String suffix)
      throws Exception {
