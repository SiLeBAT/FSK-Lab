...
//			}
			...
			if (archive.getNumEntriesWithFormat(jsonURI) == 1) {
				ArchiveEntry entry = archive.getEntriesWithFormat(jsonURI).get(0);
				File metaDataFile = FileUtil.createTempFile("metaData", ".json");
				entry.extractFile(metaDataFile);
				
				ObjectMapper objectMapper = ExtensionsKt.jacksonObjectMapper().registerModule(new RakipModule());
				portObj.genericModel = objectMapper.readValue(metaDataFile, GenericModel.class);
			}
		...
		} catch (IOException | JDOMException | ParseException | URISyntaxException e) {
			e.printStackTrace();
		}
		...
		try (RController controller = new RController()) {
			...
			// TODO: Validate model with parameter values from metadata
