...
public class MetaDataPane extends JScrollPane {
  ...
  private static class ModelSubjectEditor extends DefaultCellEditor {
    ...
    static {
      comboBox = new JComboBox<>();
      Arrays.stream(ModelClass.values())
          .forEach(modelClass -> comboBox.addItem(modelClass.fullName()));
    ...
    public Object getValueAt(int row, int col) {
      switch (Col.values()[col]) {
        ...
        case Dependent_Variable_Type:
          return template.dependentVariable.type == null ? ""
              : template.dependentVariable.type.name();
      ...
      switch (Col.values()[columnIndex]) {
        ...
        case Dependent_Variable_Type:
          template.dependentVariable.type =
              Strings.isNullOrEmpty(stringValue) ? null : DataType.valueOf(stringValue);
          break;
        case Dependent_Variable_Min:
