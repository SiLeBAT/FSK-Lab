##########################################################################################
# vertical_transmi_flocks = function(n.iter, P_initial, minimum, maximum, most_prob)
# 
# DESCRIPTION: 
...
library(triangle)
#source("vertical_transmi_flocks_params.r")
...
out <- tryCatch({P_initial=scan("InitialValues.dat")},error=function(cond){message("no file error")},warning=function(cond){message("no file warning")})
...
result <- vertical_transmi_flocks(n.iter, P_initial, minimum, maximum, most_prob) 
