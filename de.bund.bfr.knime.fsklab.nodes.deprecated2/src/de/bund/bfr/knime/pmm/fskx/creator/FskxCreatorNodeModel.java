...
public class FskxCreatorNodeModel extends ExtToolOutputNodeModel {
	...
	protected PortObject[] execute(final PortObject[] inData, final ExecutionContext exec)
		...
		{
			Map<String, String> vars = NodeUtils.getVariablesFromAssignments(portObj.param);
	...
	private static class SpreadsheetHandler {
		...
		static FskMetaData processSpreadsheet(final XSSFSheet sheet) {
			...
			final List<String> indepVars = Arrays.stream(getStringVal(sheet, Rows.indepvar.row).split("\\|\\|"))
					.map(String::trim).collect(Collectors.toList());
			final List<String> indepVarUnits = Arrays
					.stream(getStringVal(sheet, Rows.indepvar_unit.row).split("\\|\\|")).map(String::trim)
					.collect(Collectors.toList());
