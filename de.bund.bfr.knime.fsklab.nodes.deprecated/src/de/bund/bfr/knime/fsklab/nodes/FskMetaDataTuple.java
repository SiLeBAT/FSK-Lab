...
public class FskMetaDataTuple implements DataRow {
	...
	public FskMetaDataTuple(final FskMetaData template) {
		...
		// Independent variables
		{
			...
			cell[FskMetaDataFields.indepvars_units.ordinal()] = new StringCell(String.join("||", unitList));
			cell[FskMetaDataFields.indepvars_types.ordinal()] = new StringCell(String.join("||", typeList));
			...
			cell[FskMetaDataFields.indepvars_values.ordinal()] = new StringCell(String.join("||", valList));
		}
	...
	public static DataTableSpec createSpec() {
		String[] names = Arrays.stream(FskMetaDataFields.values()).map(f -> f.fullname).toArray(String[]::new);
