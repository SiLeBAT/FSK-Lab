...
*/
package de.bund.bfr.knime.testingsdk.core.ng;
...
/**
 * Configuration for a testflow. The configuration is either read from old-style configuration files in the workflows directory
 * for from a testflow configuration node inside the workflow.
 *
...
public class TestflowConfiguration {
    ...
    /**
     * Creates a new testflow configuration. The configuration is read from the testflow configuration node inside the workflow.
     * If no such node exists, configuration files are read. If no files exists, a default configuration is used.
     *
     * @param workflowManager
     *            the manager of the test workflow
     * @param globalConfiguration
     *            the global test run configuration which is used for some default values
     * @throws InvalidSettingsException
     *             if the testflow configuration node's settings cannot be read
     * @throws IOException
     ...
     * @throws InvalidSettingsException
     *             if the settings in the files are incorrect
     */
     ...
     *
     * @param statusFile
     *            the file containing the lines defining the node status after execution. If it doesn't exist, the method returns
     *            without error or without doing anything.
     * @throws IOException
     *             if the file reading fails
     * @throws InvalidSettingsException
     *             if the settings in the file are incorrect
     */
    private void readNodeStatusFromFile(final File statusFile) throws IOException, InvalidSettingsException {
        ...
        BufferedReader statusReader = new BufferedReader(
                new InputStreamReader(new FileInputStream(statusFile), Charset.forName("UTF-8")));
        ...
        while ((line = statusReader.readLine()) != null) {
            ...
            if (uppercaseLine.startsWith("ERROR") || uppercaseLine.startsWith("WARN") || uppercaseLine.startsWith("INFO")
                    || uppercaseLine.startsWith("DEBUG")) {
     ...
     *
     * @param line
     *            the line specifying the status of the node.
     * @throws InvalidSettingsException
    ...
    /**
     * From a line of the log file it extracts the part that is the interesting message. That is the part behind the first colon,
     * dismissing the date, time, level, and class info. It is important to extract the message that was sent to the logger from
     * the line.
     *
     * @param line
     ...
     *
     * @param ownerFile
     *            the file containing the owners of the test case. Each line of the file must contain one email address.
     * @throws IOException
    ...
    private void readOwnersFromFile(final File ownerFile) throws IOException {
        if (ownerFile.exists()) {
            BufferedReader r = new BufferedReader(
                    new InputStreamReader(new FileInputStream(ownerFile), Charset.forName("UTF-8")));
            String line = null;
     ...
     *
     * @param nodeId
     *            a node id relative to the current workflow manager
    ...
    /**
     * Returns the workflow timeout in seconds. After the timeout the execution of the workflow is canceled. The default timeout
     * is {@link TestrunConfiguration#DEFAULT_TIMEOUT}.
     *
     ...
     *
     * @param nodeId
     *            the node's full ID (including the workflow manager's prefix)
    ...
    /**
     * Adds a new expected node error message. This is useful in case a node has unexpectedly failed during execution and is
     * already reported in the execution sub-test.
     *
     * @param nodeId
     *            the node's ID
     * @param p
    ...
    /**
     * Adds a new required log error message. This is useful in case a node has unexpectedly failed during execution and is
     * already reported in the execution sub-test.
     *
     * @param p
     *            the error message pattern
    ...
    /**
     * Returns a human-readable string for the given pattern. If the pattern does not contain any regular expression parts, the
     * plain string is returned, otherwise the full pattern including all meta-characters is returned.
     *
     * @param pattern
     *            any pattern
