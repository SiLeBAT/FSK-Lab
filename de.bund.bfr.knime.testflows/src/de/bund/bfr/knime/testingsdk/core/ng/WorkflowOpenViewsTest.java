 ...
 */
public class WorkflowOpenViewsTest extends WorkflowTest {
    private static final NodeLogger LOGGER = NodeLogger.getLogger(WorkflowOpenViewsTest.class);

    public WorkflowOpenViewsTest(final String workflowName, final IProgressMonitor monitor, final WorkflowTestContext context) {
    ...
    private void openViews(final ErrorCollector collector, final WorkflowManager wfm) {
        for (NodeContainer node : wfm.getNodeContainers()) {
            if (node instanceof SingleNodeContainer) {
                ...
                openViews(collector, (WorkflowManager)node);
            } else {
