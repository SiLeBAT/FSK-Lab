	...
	static WorkflowManager loadWorkflow(final WorkflowTest test, final ErrorCollector collector, final File workflowDir,
		...
		WorkflowLoadHelper loadHelper = new WorkflowLoadHelper() {
			...
			public WorkflowContextV2 getWorkflowContext() {
				AnalyticsPlatformExecutorInfoBuilder execInfoBuilder = AnalyticsPlatformExecutorInfo.builder()
						.withUserId("test")
						.withLocalWorkflowPath(workflowDir.toPath())
	                    .withTempFolder(Paths.get(FileUtils.getTempDirectory().getAbsolutePath(), UUID.randomUUID().toString()));
				return WorkflowContextV2.builder()
										.withExecutor(execInfoBuilder.build())
                 .withLocation(LocalLocationInfo.getInstance(workflowDir.toPath()))
                 .build();
